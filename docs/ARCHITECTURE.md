...
- **清理原语与可靠性预留**：移除监控目录即调用 `mark_under_roots_deleted`（作为 v0.8.3 分类变更日志的“删除事件”来源）；启动归档对账挂载在 `app.rs setup` 紧随 `refresh_managed_state`；`local_file` 临时文件统一 `*.json.tmp` 供启动清理；`archive_ops` 的 source/dest/undone_at 结构即对账依据（v0.8.3 实现，本轮只固化约定）。

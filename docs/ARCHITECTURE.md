...
- **弹窗间距与表单分区约定**：编辑/设置类弹窗内容区分区统一用 `FormSection` + 父容器 `space-y-4 divide-y divide-slate-100 dark:divide-slate-800`，非首段自动出现分隔线；分区内字段间距 `space-y-2.5`、标题到内容 `mt-2.5`、字段网格 `gap-2.5`。新增弹窗沿用同一刻度，禁止手写其它间距。

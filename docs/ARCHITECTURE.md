...
- **切换控件**：`SegmentedControl` 支持 `segmented`（默认，`p-1/gap-1`）、`tabs`（底边线 + 品牌色下划线）与 `pill`（胶囊多选一，`rounded-full`）三种变体；页面主切换用 tabs（支持 `icon`/`equal`/`badge`），次级筛选用 segmented，轻量多选一（文件页视图、主题）用 pill——语义分组须传 `ariaLabel`。轻量开/关切换用 `Chip`（`active`/`selectable` 变体），禁止再手写胶囊/方块按钮组。
- **弹层栈与焦点纪律**：所有模态统一走 `Modal`（含 `ConfirmDialog`）：进程内注册表按 DOM 顺序判定栈顶，Esc 只关最顶层（嵌套确认弹窗不连锁关闭父弹窗）；打开时焦点移入面板（内容自带 `autoFocus` 的控件优先），Tab 在面板内循环，关闭后焦点还原到触发元素。确认弹窗必须是父 `Modal` 的**兄弟节点**渲染——`floating-panel` 的 `backdrop-filter` 会为 fixed 后代创建包含块，嵌在面板内会导致遮罩无法盖满视口（历史缺陷见 0.8.8 UI 审查修复）。
- **选中态与动效令牌统一**：填充式选中态一律 `bg-brand-700 text-white dark:bg-brand-500`（Chip active、SegmentedControl 徽标与 pill、图标选择器）；颜色过渡时长全局走 `--duration-fast`（`global.css` 对 `.transition-colors` 的非分层覆盖），组件内禁止再散落 150ms 默认值或硬编码时长。

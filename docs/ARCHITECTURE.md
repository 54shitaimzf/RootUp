...
- **切换控件与圆角刻度**：`SegmentedControl` 支持 `segmented`（默认，`p-1/gap-1`）与 `tabs`（底边线 + 品牌色下划线）两种变体；页面主切换用 tabs，次级筛选用 segmented。学业页圆角刻度：表格容器 0（方形）、课程卡 8px（rounded-md）、作业行/空态卡 12px（rounded-lg）、徽章 4px（rounded-sm）。

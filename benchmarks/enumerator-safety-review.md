...
- 转正（0.8.7）前需跑完整扫描回归：`mft-verify` 三档等价、真实目录 DB 对比、smoke。

## 结论

原生枚举器经加固后与 walkdir 在已知边界上行为一致（长路径、junction、错误续枚举、空目录、Unicode），并在句柄与深目录处理上更简单稳健；性能优势保留（真实目录 5.77x）。可作为 0.8.7 默认枚举器的候选，但按路线图仍保持 bench/test 门控，不进当前发布构建。

...
# 设置加载日志由前端启动后调用 get_settings 写入，需等待（快照读取可能早于前端）
$SettingsLoaded = if ($ScanDone) {
    Wait-LogLine $LogFile "settings: 加载" 60
} else {
    $false
}
Write-Result "设置加载日志" $SettingsLoaded "期望日志行 settings: 加载（前端启动后写入）"
...
Write-Result "快速扫描降级回退日志" ($LogContent -match "scan: 快速扫描不可用 dir=$TestDir") "默认构建未启用 ROOTUP_MFT_SCAN，应回退 walkdir 并记录原因"
Write-Result "USN 补账降级/基线日志" ($LogContent -match "usn: (补账不可用|基线不可用|建立基线)") "无提权环境应记录 usn 降级或基线日志"
